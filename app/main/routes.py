...
@bp.route('/edit/project/<int:project_id>', methods = ["GET", "POST"])
@login_required
def edit_project(project_id) :
    ...
    if current_user.is_in_project(project_id) :
        ...
        return render_template('main/edit_project.html', form = form, project_name = project.name, project_id = project_id, project = project)
    else :
        flash("You cannot edit this project.")
        return redirect(url_for('main.index'))

@bp.route('/delete/project/<int:project_id>', methods = ["GET", "POST"])
@login_required
...
@bp.route('/new/tasklist/<int:project_id>', methods = ["GET", "POST"])
@login_required
def new_tasklist(project_id) :
    ...
    if form.validate_on_submit() :
        ...
        if current_user.is_in_project(project_id) :
            ...
            return redirect(url_for('main.index'))
    return render_template('main/new_tasklist.html', form = form, project_id = project_id, project_name = project.name)

@bp.route('/edit/project/<int:project_id>/tasklist/<int:tasklist_id>', methods = ["GET", "POST"])
@login_required
def edit_tasklist(project_id, tasklist_id) :
    ...
    if current_user.is_in_project(project_id) and tasklist.project_id == project_id :
        ...
        return render_template('main/edit_tasklist.html', form = form, project_id = project_id, tasklist_id = tasklist_id, project_name = project.name, tasklist_name = tasklist.name, tasklist = tasklist)
    else :
        flash("You cannot edit task lists for this project.")
        return redirect(url_for('main.index'))

@bp.route('/delete/project/<int:project_id>/tasklist/<int:tasklist_id>', methods = ["GET", "POST"])
@login_required
...
@bp.route('/new/project/<int:project_id>/tasklist/<int:tasklist_id>', methods = ["GET", "POST"])
@login_required
def new_task(project_id, tasklist_id) :
    ...
    if form.validate_on_submit() :
        ...
        if current_user.is_in_project(project_id) and tasklist.project_id == project_id :
            ...
            return redirect(url_for('main.index'))
    return render_template('main/new_task.html', form = form, project_id = project_id, tasklist_id = tasklist_id, project_name = project.name, tasklist_name = tasklist.name)

@bp.route('/edit/project/<int:project_id>/tasklist/<int:tasklist_id>/task/<int:task_id>', methods = ["GET", "POST"])
@login_required
    ...
    if current_user.is_in_project(project_id) and tasklist.project_id == project_id and task.tasklist_id == tasklist_id :
        if form.validate_on_submit() :
            ...
            if new_tasklist.project_id != project_id :
                ...
                return redirect(url_for('main.edit_task', project_id = project_id, tasklist_id = tasklist_id, task_id = task_id))
            else :
        ...
        return render_template('main/edit_task.html', form = form, project_id = project_id, tasklist_id = tasklist_id, task_id = task_id, project_name = project.name, tasklist_name = tasklist.name, task = task)
    else :
        flash("You cannot edit tasks for this project.")
        return redirect(url_for('main.index'))

@bp.route('/delete/project/<int:project_id>/tasklist/<int:tasklist_id>/task/<int:task_id>', methods = ["GET", "POST"])
@login_required
...
@bp.route('/new/project/<int:project_id>/tasklist/<int:tasklist_id>/task/<int:task_id>', methods = ["GET", "POST"])
@login_required
    ...
    if form.validate_on_submit() :
        ...
        if current_user.is_in_project(project_id) and tasklist.project_id == project_id and task.tasklist_id == tasklist_id :
            ...
            if time_diff.total_seconds() <= 0 :
                ...
                return redirect(url_for('main.new_tasklog', project_id = project_id, tasklist_id = tasklist_id, task_id = task_id))
            else :
                ...
                return redirect(url_for('main.edit_task', project_id = project.id, tasklist_id = tasklist_id, task_id = task_id))
        else :
            flash("You cannot add task logs to this project.")
            return redirect(url_for('main.index'))
    return render_template('main/new_tasklog.html', form = form, project_id = project.id, tasklist_id = tasklist_id, task_id = task_id, project_name = project.name, tasklist_name = tasklist.name, task_name = task.name)

@bp.route('/edit/project/<int:project_id>/tasklist/<int:tasklist_id>/task/<int:task_id>/tasklog/<int:tasklog_id>', methods = ["GET", "POST"])
@login_required
def edit_tasklog(project_id, tasklist_id, task_id, tasklog_id) :
    ...
    if current_user.is_in_project(project_id) and tasklist.project_id == project_id and task.tasklist_id == tasklist_id and tasklog.task_id == task_id :
        ...
        return render_template('main/edit_tasklog.html', form = form, project_id = project_id, tasklist_id = tasklist_id, task_id = task_id, tasklog_id = tasklog_id, project_name = project.name, tasklist_name = tasklist.name, task_name = task.name, tasklog_name = tasklog.note)
    else :
        flash("You cannot edit task logs for this project.")
        return redirect(url_for('main.index'))

@bp.route('/delete/project/<int:project_id>/tasklist/<int:tasklist_id>/task/<int:task_id>/tasklog/<int:tasklog_id>', methods = ["GET", "POST"])
@login_required
